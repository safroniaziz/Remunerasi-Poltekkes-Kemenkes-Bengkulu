        ...
        <div class="col-md-12">
            <section class="panel" style="margin-bottom:20px;">
                <header class="bg-primary" style="color: #ffffff;background-color: #3c8dbc;border-color: #fff000;border-image: none;border-style: solid solid none;border-width: 4px 0px 0;border-radius: 0;font-size: 14px;font-weight: 700;padding: 15px;">
                    <i class="fa fa-users"></i>&nbsp; Manajemen Data Rubrik 19 Memberi Pelatihan Penyuluhan Penataran Ceramah kepada masyarakat
                </header>
                        ...
                        <div class="col-md-12">
                            <div style="margin-bottom: 10px !important;">
                                <button type="button" class="btn btn-primary btn-sm btn-flat" data-toggle="modal" data-target="#modal-default">
                                    <i class="fa fa-plus"></i>&nbsp; Tambah Rubrik 19 Memberi Pelatihan Penyuluhan Penataran Ceramah kepada masyarakat
                                </button>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <table class="table table-striped table-bordered" id="table" style="width:100%;">
                                ...
                                <tbody>
                                    @php
                                        $no=1;
                                    @endphp
                                    @foreach ($r019latihnyuluhnatarceramahwargas as $index => $r019latihnyuluhnatarceramahwarga)
                                        <tr>
                                            <td>{{ $index+1 }}</td>
                                            ...
                                            <td class="text-center">
                                                @if ($r019latihnyuluhnatarceramahwarga->is_verified == 1)
                                                    <form action="{{ route('r_019_latih_nyuluh_natar_ceramah_warga.tolak',[$r019latihnyuluhnatarceramahwarga->id]) }}" method="POST">
                                                        {{ csrf_field() }} {{ method_field('PATCH') }}
                                                        <button type="submit" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-thumbs-up"></i></button>
                                                    </form>
                                                @else
                                                    <form action="{{ route('r_019_latih_nyuluh_natar_ceramah_warga.verifikasi',[$r019latihnyuluhnatarceramahwarga->id]) }}" method="POST">
                                                        {{ csrf_field() }} {{ method_field('PATCH') }}
...
@push('scripts')
    ...
    <script>
        ...
        function editr019latihnyuluhnatarceramahwarga(id){
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            url = "{{ url('r_019_latih_nyuluh_natar_ceramah_warga').'/' }}"+id+'/edit';
            $.ajax({
                url : url,
                type : 'GET',
