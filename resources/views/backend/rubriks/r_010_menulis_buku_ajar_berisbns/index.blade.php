            ...
            <section class="panel" style="margin-bottom:20px;">
                ...
                <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
                    <div class="row" style="margin-right:-15px; margin-left:-15px;">
                            ...
                            <table class="table table-striped table-bordered" id="table" style="width:100%; m">
                                ...
                                <tbody>
                                    @php
                                        $no=1;
                                    @endphp
                                    @foreach ($r010menulisbukuajarberisbns as $index => $r010menulisbukuajarberisbn)
                                        <tr>
                                            <td>{{ $index+1 }}</td>
                                            <td class="text-center">{{ $r010menulisbukuajarberisbn->nip }}</td>
                                            <td class="text-center">{{ $r010menulisbukuajarberisbn->pegawai->nama }}</td>
                                            <td class="text-center">{{ $r010menulisbukuajarberisbn->judul }}</td>
                                            ...
                                            <td class="text-center">{{ $r010menulisbukuajarberisbn->point }}</td>
                                            <td></td>
                                            <td></td>
                                            {{--  <td>
                                                @if ($r010menulisbukuajarberisbn->is_bkd== 1)
                                                    <form action="{{ route('r_10_menulis_buku_ajar_berisbn.set_nonactive',[$r010menulisbukuajarberisbn->id]) }}" method="POST">
                                                        {{ csrf_field() }} {{ method_field('PATCH') }}
                                                        <button type="submit" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-thumbs-up"></i></button>
                                                    </form>
                                                @else
                                                    <form action="{{ route('r_10_menulis_buku_ajar_berisbn.set_active',[$r010menulisbukuajarberisbn->id]) }}" method="POST">
                                                        {{ csrf_field() }} {{ method_field('PATCH') }}
                                           ...
                                           {{--  <td>
                                            @if ($r010menulisbukuajarberisbn->is_verified== 1)
                                                <form action="{{ route('r01_perkuliahan_teori.set_nonactive',[$r010menulisbukuajarberisbn->id]) }}" method="POST">
                                                    {{ csrf_field() }} {{ method_field('PATCH') }}
                                                    <button type="submit" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-thumbs-up"></i></button>
                                                </form>
                                            @else
                                                <form action="{{ route('r01_perkuliahan_teori.set_active',[$r010menulisbukuajarberisbn->id]) }}" method="POST">
                                                    {{ csrf_field() }} {{ method_field('PATCH') }}
                                                    <button type="submit" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-thumbs-down"></i></button>
                                                </form>
                                            @endif
                        ...
                        @include('backend/rubriks/r_010_menulis_buku_ajar_berisbns.partials.modal_add')
                    </div>
                    @include('backend/rubriks/r_010_menulis_buku_ajar_berisbns.partials.modal_edit')
                </div>
            </section>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function editr010menulisbukuajarberisbn(id){
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            url = "{{ url('r_010_menulis_buku_ajar_berisbn').'/' }}"+id+'/edit';
            $.ajax({
                url : url,
                type : 'GET',
                success : function(data){
                    $('#modalEdit').modal('show');
                    $('#r010menulisbukuajarberisbn_id_edit').val(data.id);
