                                    ...
                                    @foreach ($r07membimbingskripsiltalaprofesis as $index => $r07membimbingskripsiltalaprofesi)
                                        <tr>
                                            ...
                                            @if(Auth::user()->hasRole('verifikator'))
                                                ...
                                                <td class="text-center">
                                                    @if ($r07membimbingskripsiltalaprofesi->is_verified == 1)
                                                        <form action="{{ route('r_07_membimbing_skripsi_lta_la_profesi.tolak',[$r07membimbingskripsiltalaprofesi->id]) }}" method="POST">
                                                            {{ csrf_field() }} {{ method_field('PATCH') }}
                                                            <button type="submit" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-thumbs-up"></i></button>
                                                        </form>
                                                    @else
                                                        <form action="{{ route('r_07_membimbing_skripsi_lta_la_profesi.verifikasi',[$r07membimbingskripsiltalaprofesi->id]) }}" method="POST">
                                                            {{ csrf_field() }} {{ method_field('PATCH') }}
