                                    ...
                                    @foreach ($r023auditormutuassessorakredinternals as $index => $r023auditormutuassessorakredinternal)
                                        <tr>
                                            ...
                                            @if(Auth::user()->hasRole('verifikator'))
                                                ...
                                                <td class="text-center">
                                                    @if ($r023auditormutuassessorakredinternal->is_verified == 1)
                                                        <form action="{{ route('r_023_auditor_mutu_assessor_akred_internal.tolak',[$r023auditormutuassessorakredinternal->id]) }}" method="POST">
                                                            {{ csrf_field() }} {{ method_field('PATCH') }}
                                                            <button type="submit" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-thumbs-up"></i></button>
                                                        </form>
                                                    @else
                                                        <form action="{{ route('r_023_auditor_mutu_assessor_akred_internal.verifikasi',[$r023auditormutuassessorakredinternal->id]) }}" method="POST">
                                                            {{ csrf_field() }} {{ method_field('PATCH') }}
