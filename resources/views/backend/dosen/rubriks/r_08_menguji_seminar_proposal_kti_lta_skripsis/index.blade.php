@extends('layouts.app')
@section('subTitle','Data Rubrik 08 Menguji Seminar Proposal KTI LTA Skripsi')
        ...
        <div class="col-md-12">
            <section class="panel" style="margin-bottom:20px;">
                <header class="bg-primary" style="color: #ffffff;background-color: #3c8dbc;border-color: #fff000;border-image: none;border-style: solid solid none;border-width: 4px 0px 0;border-radius: 0;font-size: 14px;font-weight: 700;padding: 15px;">
                    <i class="fa fa-users"></i>&nbsp; Manajemen Data Rubrik 08 Menguji Seminar Proposal KTI LTA Skripsi
                </header>
                <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
                    <div class="row" style="margin-right:-15px; margin-left:-15px;">
                            ...
                            <table class="table table-striped table-bordered" id="table" style="width:100%;">
                                ...
                                <tbody>
                                    @php
                                        $no=1;
                                    @endphp
                                    @foreach ($r08mengujiseminarproposalktiltaskripsis as $index => $r08mengujiseminarproposalktiltaskripsi)
                                        <tr>
                                            <td>{{ $index+1 }}</td>
                                            <td class="text-center">{{ $r08mengujiseminarproposalktiltaskripsi->nip }}</td>
                                            ...
                                            <td>
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <a onclick="editr08mengujiseminarproposal({{ $r08mengujiseminarproposalktiltaskripsi->id }})" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-edit"></i>&nbsp; Edit</a>
                                                        </td>
                                                        <td>
                                                            <form action="{{ route('dosen.r_08_menguji_seminar_proposal_kti_lta_skripsi.delete',[$r08mengujiseminarproposalktiltaskripsi->id]) }}" method="POST" id="form-hapus">
                        ...
                        @include('backend/dosen/rubriks/r_08_menguji_seminar_proposal_kti_lta_skripsis.partials.modal_add')
                    </div>
                    @include('backend/dosen/rubriks/r_08_menguji_seminar_proposal_kti_lta_skripsis.partials.modal_edit')
                </div>
            </section>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function editr08mengujiseminarproposal(id){
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            url = "{{ url('dosen/r_08_menguji_seminar_proposal_kti_lta_skripsi').'/' }}"+id+'/edit';
            $.ajax({
                url : url,
                type : 'GET',
